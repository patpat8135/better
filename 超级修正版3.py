...
# 页面标题
st.title("内容传播效果预测工具 (完整版修正版 CrowdTest)")
...
if st.button("开始测试", key="start_test"):
    if content_a.strip() == "" or content_b.strip() == "":
        ...
        with col1:
            ...
            if st.button("一键改写内容 A (生成3版)", key="rewrite_a"):
        ...
        with col2:
            ...
            if st.button("一键改写内容 B (生成3版)", key="rewrite_b"):
...
# 页脚
st.markdown("---")
st.caption("版本：MVP 超级修正版 🚀 | 功能：传播模拟 + AI改写 + 潜力分析 + 完美修复按钮问题")
